...
from maskrcnn_benchmark.engine.trainer import do_mask_da_train, do_train
...
def train(cfg, local_rank, distributed):
    ...
    extra_checkpoint_data = checkpointer.load(cfg.MODEL.WEIGHT, optim=True)
    ...
    if cfg.MODEL.DOMAIN_ADAPTATION_ON:
        source_data_loader, _ = make_data_loader(
            cfg,
            is_train=True,
            is_source=True,
        ...
        target_data_loader, _ = make_data_loader(
            cfg,
            is_train=True,
            is_source=False,
            is_distributed=distributed,
        ...
        data_loader_val = make_data_loader(cfg, is_train=False, is_distributed=distributed)[0][0]
        ...
        if cfg.MODEL.MIC_ON:
            ...
            # build stuff related to the teacher 
            model_t = build_detection_model(cfg)
            model_t.to(device)
            if distributed:
                model_t = torch.nn.parallel.DistributedDataParallel(
                        model_t, device_ids=[local_rank], output_device=local_rank,find_unused_parameters=True,
                        # this should be removed if we update BatchNorm stats
                        broadcast_buffers=False,
                )
            ...
            if cfg.MODEL.TEACHER_WEIGHT is not None:
                extra_checkpoint_data = checkpointer_teacher.load(cfg.MODEL.TEACHER_WEIGHT)
